...
**Summary**: Use the PostgreSQL `HAVING` clause to specify a search condition for a group or an aggregate.
...
- First, the `GROUP BY` clause groups rows into groups by the values in `column1`.
...
In other words, you apply the condition in the `WHERE` clause to the rows while you apply the condition in the `HAVING` clause to groups of rows.

## PostgreSQL `HAVING` clause examples

Check the `payment` table in the sample database:

![Check 'payment' table in sample database](img/03.png)
...
See the `customer` table from the sample database:

![Check customer table](img/04.png)
