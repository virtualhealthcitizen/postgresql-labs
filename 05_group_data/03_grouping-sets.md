...
**Summary**: Understand grouping sets and how to use the PostgreSQL `GROUPING SETS` clause to generate multiple grouping sets in a query.

## Set up a sample table

Create a new table called `sales` for demonstration purposes.
...
For example, the following query uses the `GROUP BY` clause to return the number of products sold by brand and segment.
In other words, it defines a grouping set of the brand and segment which is denoted by `(brand, segment)`.
...
The following query finds the number of products sold by a brand.
...
The following query finds the number if products sold by segment.
...
Suppose you want to get all the grouping sets using a single query.
To achieve this, you can use `UNION ALL` to combine all the result sets of the queries above.
...
In this syntax, there are four grouping sets:
...
To apply this syntax to the above example, you can use the `GROUPING SETS` clause instead of the `UNION ALL` clause like this:
...
In addition, PostgreSQL will optimize the number of times it scans the `sales` table and will not scan multiple times.
...
The `GROUPING()` function returns bit `0` if the ar gument is a member of the current grouping set and `1` otherwise.
...
As shown in the above screenshot, when the value in the `grouping_brand` is `0`, the `sum` column shows the subtotal of the `brand`.
...
Use PostgreSQL `GROUPING SETS` to generate multiple grouping sets.
