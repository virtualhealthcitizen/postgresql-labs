...
The query returns only rows that satisfy the `condition` in the `WHERE` clause. In other words, only rows that cause the `condition` to evaluate to `true` will be included in the result set.
...
Let's practice with some examples of using the `WHERE` clause. We will use the `customer` table from the `dvdrental` sample database for demonstration.
