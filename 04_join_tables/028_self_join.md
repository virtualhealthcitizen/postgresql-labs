...
**Summary**: In this section, you will learn how to use the PostgreSQL self-join technique to compare rows within the same table.
...
### Using Self-Join to Find Who Reports to Whom
...
This query references the `employee` table twice, one as the employee and the other as the manager.
...
### Including the Top Manager in the Result Set

```sql
SELECT
    e.first_name || ' ' || e.kast_name employee,
    m.first_name || ' ' || m.last_name manager
FROM
    employee e
...
### Finding All Pairs of Films with the Same Length
