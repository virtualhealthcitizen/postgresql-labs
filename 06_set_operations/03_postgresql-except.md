...
**Summary**: Understand how to use the PostgreSQL `EXCEPT` operator to return a result set containing the rows in the first query that do not appear in the output of the second query.
...
- The number of columns and their order must be the same in the two queries.
- The data types of the respective columns must be compatible.

The following Venn diagram illustrates the `EXCEPT` operator:

![`EXCEPT` operator Venn diagram](img/02_venn-diagram_except_01.png)
...
Create the `top_rated_films` and `most_popular_films` tables for demonstration:
...
Below are some examples of using the `EXCEPT` operator.
...
Notice that the `ORDER BY` clause was placed after the second query to sort the films by title.
