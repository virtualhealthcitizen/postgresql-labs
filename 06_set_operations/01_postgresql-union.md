...
**Summary**: Understand PostgreSQL `UNION` operator to combine result sets of multiple queries into a single result set.
...
In this syntax, the queries must confirm to the following rules:

- The number and order of the columns in the select list of both queries must be the same.
- The data types of the columns in the select lists of the queries must be compatible.
...
The following statement uses the `UNION` operator to combine data from the queries that retrieve data from the `top_rated_films` and `most_popular_films` tables:

```sql
SELECT * FROM top_rated_films
UNION
SELECT * FROM most_popular films;
```
...
The following statement uses the `UNION ALL` operator to combine result sets from queries that retrieve data from the `top_rated_films` and `most_popular_films` tables:
...
The output indicates that the `UNION ALL` operator returns the duplicate rows.
...
- Use the `UNION` operator to combine result sets of two queries and return distinct rows.
- Use the `UNION ALL` operator to combine the result sets of two queries but retain the duplicate rows.
