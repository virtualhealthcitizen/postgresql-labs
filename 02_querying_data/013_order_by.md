...
- Second, you use the `ASC` option to sort rows in ascending order and the `DESC` option to sort rows in descending order.
  If you omit the `ASC` or `DESC` option, the `ORDER BY` clause uses `ASC` by default.
...
In this example, the `ORDER BY` clause sorts rows by values in the `first_name` column first.
And then it sorts the sorted rows by values in the `last_name` column.
...
So if you use the `ASC` option, the `ORDER BY` clause uses the `NULLS LAST` option by default.
Therefore, the following query returns the same result:
